...
use super::detect::{
    detect_proton_in_folder, detect_wine_in_folder, validate_and_get_wine_version,
};
use super::steam::{get_steam_compat_paths, get_steam_proton_paths};
...
fn get_system_wine() -> Vec<WineVersion> {
...
fn get_steam_proton() -> Vec<WineVersion> {
    let mut versions = Vec::new();

    for common_path in get_steam_proton_paths() {
        let source = if common_path
            .to_str()
            .unwrap_or("")
            .contains(".var/app/com.valvesoftware.Steam")
        {
...
fn get_lutris_wine() -> Vec<WineVersion> {
    let mut versions = Vec::new();
...
fn get_bottles_wine() -> Vec<WineVersion> {
    let mut versions = Vec::new();
...
pub fn get_all_wine_versions() -> Vec<WineVersion> {
    ...
    versions.extend(get_installed_proton());
